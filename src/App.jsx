...
export default function WordSearch() {
  const [screen,setScreen]=useState('picker');       // picker | difficulty | game | gameover | timeout | done
  const [showIdx,setShowIdx]=useState(null);
  const [diff,setDiff]=useState(null);               // 'easy'|'hard'|'expert'
  ...
  const [penaltyMsg,setPenaltyMsg]=useState(null);   // e.g. "−10s"
  ...
  const timerRef=useRef(null),gridContRef=useRef(null),timeLeftRef=useRef(0);
  ...
  // Cell sizing based on difficulty grid size
  ...
  const T=dk?{
    ...
    taglineColor:'#D4A800',
    pillBg:'rgba(255,255,255,0.07)',pillText:'#BBB',
  ...
  useEffect(()=>{
    ...
    timerRef.current=setInterval(()=>{
      setTimeLeft(t=>{
        ...
        if(next<=0){clearInterval(timerRef.current);setScreen('timeout');}
        return next;
      });
    },1000);
  ...
  function startGame(idx,d){
    ...
    const{grid,placed}=buildGrid(s.words,cfg.gridSize);
    gRef.current={grid,placed};
    setShowIdx(idx);setDiff(d);setGameData({grid,placed});setFound(new Set());
    setSel(null);setTimeLeft(cfg.time);timeLeftRef.current=cfg.time;
    setFlash(null);setPenaltyMsg(null);setRevealedWord(null);
    setPoints(0);setRevealMode(false);setConfetti(false);
    setScreen('game');
  }
  ...
  const commit=useCallback(()=>{
    ...
    if(match){
      ...
      // Wrong drag — apply time penalty
      const pen=diffCfg?.penalty||10;
      setTimeLeft(t=>{
        const next=Math.max(0,t-pen);
        timeLeftRef.current=next;
        if(next<=0){clearInterval(timerRef.current);setTimeout(()=>setScreen('timeout'),300);}
        return next;
      });
      setPenaltyMsg('−'+pen+'s');
      setTimeout(()=>setPenaltyMsg(null),900);
  ...
  function handlePillClick(word){
    ...
    if(revealMode){
      ...
      setPoints(p=>p-(diffCfg?.revealCost||8));
      setRevealMode(false);
    }else{
      clearInterval(timerRef.current);setRevealedWord(word);setScreen('gameover');
    }
  }
  ...
  // Timer colour: white > 60s, orange 30-60s, red < 30s, pulsing < 10s
  ...
  const ToggleBtn=()=>(
    <button onClick={()=>setDarkMode(d=>!d)} title="Toggle theme"
      style={{background:T.btnBg,border:'1px solid '+T.btnBorder,borderRadius:20,padding:'5px 11px',cursor:'pointer',fontSize:'0.88rem',lineHeight:1,flexShrink:0}}>
      {dk?'☀️':'🌙'}
    </button>
  );
    ...
    <div style={{minHeight:'100vh',background:T.pageBg,fontFamily:"'Georgia',serif",color:T.text,display:'flex',flexDirection:'column',alignItems:'center',transition:'background 0.35s,color 0.25s',padding:isMobile?'0 8px 28px':'0 14px 36px',overflowX:'hidden'}}>
        ...
        <div style={{width:'100%',maxWidth:980,animation:'fadeIn 0.5s ease'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:isMobile?'20px 0 4px':'28px 0 6px'}}>
            ...
            <div style={{textAlign:'center',flex:2}}>
              ...
              <div style={{fontFamily:"'Crimson Text',serif",color:T.muted,fontSize:'0.8rem',marginTop:6,fontStyle:'italic'}}>{SHOWS.length} titles · choose one to begin</div>
            </div>
                ...
                {section.ids.map(id=>{
                  ...
                  return(
                    <div key={s.id} className="show-card" onClick={()=>{setShowIdx(i);setScreen('difficulty');}}>
                      ...
                      <div style={{fontSize:'0.62rem',color:T.muted}}>15 words</div>
                    </div>
                  );
                })}
              </div>
      ...
      {screen==='difficulty'&&show&&(
        <div style={{width:'100%',maxWidth:520,animation:'fadeIn 0.35s ease',padding:isMobile?'30px 0':'50px 0'}}>
          ...
          <div style={{textAlign:'center',marginBottom:32}}>
            ...
            <div style={{fontFamily:"'Crimson Text',serif",color:T.muted,fontSize:'0.88rem',marginTop:6,fontStyle:'italic'}}>Choose your difficulty</div>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            {Object.entries(DIFF).map(([key,cfg])=>(
              <div key={key} className="diff-card" onClick={()=>startGame(showIdx,key)}
        ...
        <div style={{width:'100%',maxWidth:1100,animation:'fadeIn 0.4s ease'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,padding:isMobile?'12px 0 8px':'18px 0 10px'}}>
            ...
            <div style={{textAlign:'center',flex:1,overflow:'hidden'}}>
              <div style={{fontFamily:"'Cinzel',serif",fontSize:isMobile?'0.9rem':'clamp(1rem,2.5vw,1.5rem)',fontWeight:700,color:accentColor,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                {show.emoji} {show.title}
              </div>
              <div style={{fontFamily:"'Crimson Text',serif",fontSize:'0.65rem',color:DIFF[diff].color,letterSpacing:'0.1em',marginTop:2,textTransform:'uppercase'}}>{diffCfg.label}</div>
            </div>
            {/* Countdown */}
            <div style={{fontFamily:"'Cinzel',serif",fontSize:isMobile?'1.1rem':'1.3rem',color:timerColor,flexShrink:0,minWidth:54,textAlign:'right',animation:timerAnim,fontWeight:700}}>
              {fmtTime(timeLeft)}
            </div>
            <ToggleBtn/>
          </div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:isMobile?10:12,flexWrap:'wrap'}}>
            ...
            {points>=revealCost?(
              <button onClick={()=>setRevealMode(r=>!r)} style={{
                ...
                fontFamily:"'Cinzel',serif",fontSize:'0.75rem',fontWeight:700,letterSpacing:'0.03em',
                boxShadow:revealMode?'0 0 14px '+accentColor+'88':'none',
                transition:'all 0.2s',animation:revealMode?'pulse 1.2s infinite':'none',
              }}>
                💡 {revealMode?`Tap a word to reveal (−${revealCost}pts)`:`Reveal (${revealCost}pts)`}
              </button>
            ):(
              <div style={{fontFamily:"'Crimson Text',serif",fontSize:'0.7rem',color:T.subtle,fontStyle:'italic'}}>
                {revealCost-points} more {revealCost-points===1?'point':'points'} to unlock a reveal
              </div>
            )}
          </div>
          ...
          <div style={{display:'flex',flexDirection:isMobile?'column':'row',gap:isMobile?14:22,alignItems:'flex-start'}}>
            {/* Grid wrapper — penalty message overlay */}
            <div style={{position:'relative',display:'flex',justifyContent:isMobile?'center':'flex-start',flexShrink:0,width:isMobile?'100%':'auto'}}>
              {penaltyMsg&&(
                <div style={{position:'absolute',top:'50%',left:'50%',zIndex:10,fontFamily:"'Cinzel',serif",fontSize:isMobile?'1.6rem':'2rem',fontWeight:900,color:'#E74C3C',textShadow:'0 0 20px rgba(231,76,60,0.8)',pointerEvents:'none',animation:'penaltyFade 0.9s ease forwards'}}>
                  {penaltyMsg}
                </div>
              )}
            ...
            <div style={{flex:1,minWidth:0,width:'100%'}}>
              <div style={{fontFamily:"'Cinzel',serif",fontSize:'0.6rem',letterSpacing:'0.14em',color:T.subtle,marginBottom:8}}>
                  ...
                  return(
                    <div key={p.word} className={'word-pill'+(isFound?' found-pop':'')}
                      ...
                      style={{
                        ...
                        transform:revealMode&&!isFound?'scale(1.04)':'scale(1)',
                      }}>
                      {p.word}
                    </div>
                  );
      ...
      {screen==='timeout'&&show&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'80vh',gap:16,animation:'fadeIn 0.5s ease',textAlign:'center',padding:'24px'}}>
          ...
          <div style={{fontFamily:"'Crimson Text',serif",color:T.muted,fontSize:'1.05rem',fontStyle:'italic'}}>{show.emoji} {show.title}</div>
          <div style={{fontFamily:"'Crimson Text',serif",color:T.subtle,fontSize:'0.9rem'}}>{found.size} of {gameData?.placed.length} words found on {diffCfg?.label}</div>
          <div style={{display:'flex',gap:12,marginTop:10,flexWrap:'wrap',justifyContent:'center'}}>
            <button onClick={()=>startGame(showIdx,diff)} style={{background:'rgba(230,126,34,0.15)',border:'1px solid rgba(230,126,34,0.55)',color:'#E67E22',padding:'10px 22px',borderRadius:8,fontFamily:"'Cinzel',serif",fontSize:'0.88rem',cursor:'pointer',letterSpacing:'0.05em'}}>Try Again</button>
            <button onClick={()=>setScreen('difficulty')} style={{background:T.btnBg,border:'1px solid '+T.btnBorder,color:T.btnText,padding:'10px 22px',borderRadius:8,fontFamily:"'Cinzel',serif",fontSize:'0.88rem',cursor:'pointer',letterSpacing:'0.05em'}}>Change Difficulty</button>
      ...
      {screen==='gameover'&&show&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'80vh',gap:16,animation:'fadeIn 0.5s ease',textAlign:'center',padding:'24px'}}>
          ...
          <div style={{display:'flex',gap:12,marginTop:10,flexWrap:'wrap',justifyContent:'center'}}>
            <button onClick={()=>startGame(showIdx,diff)} style={{background:'rgba(192,57,43,0.15)',border:'1px solid rgba(192,57,43,0.55)',color:'#C0392B',padding:'10px 22px',borderRadius:8,fontFamily:"'Cinzel',serif",fontSize:'0.88rem',cursor:'pointer',letterSpacing:'0.05em'}}>Try Again</button>
            <button onClick={()=>setScreen('picker')} style={{background:T.btnBg,border:'1px solid '+T.btnBorder,color:T.btnText,padding:'10px 22px',borderRadius:8,fontFamily:"'Cinzel',serif",fontSize:'0.88rem',cursor:'pointer',letterSpacing:'0.05em'}}>Choose Another</button>
          </div>
          <Footer/>
        </div>
      ...
      {screen==='done'&&show&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'80vh',gap:16,animation:'fadeIn 0.6s ease',textAlign:'center',padding:'24px'}}>
          ...
          <div style={{display:'flex',gap:12,marginTop:10,flexWrap:'wrap',justifyContent:'center'}}>
            <button onClick={()=>startGame(showIdx,diff)} style={{background:accentColor+'22',border:'1px solid '+accentColor+'88',color:accentColor,padding:'10px 22px',borderRadius:8,fontFamily:"'Cinzel',serif",fontSize:'0.88rem',cursor:'pointer',letterSpacing:'0.05em'}}>Play Again</button>
            <button onClick={()=>setScreen('difficulty')} style={{background:T.btnBg,border:'1px solid '+T.btnBorder,color:T.btnText,padding:'10px 22px',borderRadius:8,fontFamily:"'Cinzel',serif",fontSize:'0.88rem',cursor:'pointer',letterSpacing:'0.05em'}}>Change Difficulty</button>
            <button onClick={()=>setScreen('picker')} style={{background:T.btnBg,border:'1px solid '+T.btnBorder,color:T.btnText,padding:'10px 22px',borderRadius:8,fontFamily:"'Cinzel',serif",fontSize:'0.88rem',cursor:'pointer',letterSpacing:'0.05em'}}>Choose Another</button>
          </div>
          <Footer/>
